...
class TestComputePvs:
    ...
    def test_no_pay_penalized(self) -> None:
        ...
        score = compute_pvs(
            job, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
        assert score <= 0.25
    ...
    def test_with_pay_scores_higher(self) -> None:
        ...
        score_pay = compute_pvs(
            job_pay, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
        score_no_pay = compute_pvs(
            job_no_pay, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
    ...
    def test_nearby_scores_higher(self) -> None:
        ...
        score_near = compute_pvs(
            job_near, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
        score_far = compute_pvs(
            job_far, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
    ...
    def test_transit_penalty_lowers_score(self) -> None:
        ...
        score_car = compute_pvs(
            job, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
        score_bus = compute_pvs(
            job, user_zip="36101", transit_dependent=True,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
    ...
    def test_schedule_conflict_reduces_score(self) -> None:
        ...
        score_flex = compute_pvs(
            job, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE, barriers=[],
        )
        score_day = compute_pvs(
            job, user_zip="36101", transit_dependent=False,
    ...
    def test_credit_blocked_lowers_barrier_compat(self) -> None:
        ...
        score_ok = compute_pvs(
            job_ok, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE,
            barriers=[BarrierType.CREDIT],
        )
        score_blocked = compute_pvs(
            job_blocked, user_zip="36101", transit_dependent=False,
            schedule_type=AvailableHours.FLEXIBLE,
            barriers=[BarrierType.CREDIT],
        )
